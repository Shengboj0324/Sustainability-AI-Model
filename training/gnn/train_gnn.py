...
def load_graph_data(config: dict) -> Data:
    """
    Load the canonical sustainability Knowledge Graph.

    The primary path consumes processed parquet generated by
    scripts/data_pipeline_stress_validate.py. If explicitly allowed, fallback
    graph generation uses models.gnn.graph_contract so training never silently
    diverges from the vision taxonomy.
    """
    ...
    logger.info("Building canonical sustainability Knowledge Graph from graph_contract.py...")
    edge_df, node_df = build_graph_tables(int(config["model"]["input_dim"]))
    feature_cols = sorted(
        [c for c in node_df.columns if c.startswith("feature_")],
        key=lambda c: int(c.split("_", 1)[1]),
    )
    edge_index = torch.tensor(edge_df[["source", "target"]].astype("int64").to_numpy().T, dtype=torch.long)
    x = torch.tensor(node_df[feature_cols].astype("float32").to_numpy(), dtype=torch.float32)
    data = Data(x=x, edge_index=edge_index, num_nodes=x.size(0))
    logger.info("Canonical Knowledge Graph: %d nodes, %d edges", x.size(0), edge_index.size(1))
    return data
